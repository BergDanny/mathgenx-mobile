...
const varkQuestions = ref([]);
const selectedAnswers = ref({});
const isSubmitting = ref(false);
const toast = inject('toast');


async function loadVarkQuestions() {
    loading.value = true;
    error.value = null;
    try {
        const res = await getVarkQuestions();
        if (res.success) {
            varkQuestions.value = res.data || [];
            // mirror into the profiling store so validation can read questions
            profilingStore.varkQuestions = res.data || [];
        } else {
            error.value = res.message || "Failed to load questions.";
        }
    } catch (err) {
        error.value = "An unexpected error occurred while loading questions.";
        console.error(err);
    } finally {
        loading.value = false;
    }
...
function selectAnswer(questionId, answerId) {
    profilingStore.saveVarkResponse(questionId, answerId);
}
onMounted(async () => {
    await loadVarkQuestions();
...
<template>
    <div class="max-w-5xl mx-auto py-4 sm:py-6 md:py-10 px-2 sm:px-4 pt-0">
        ...
        <div v-else class="space-y-3 sm:space-y-4">
            <div v-for="(question, index) in profilingStore.varkQuestions" :key="question.id"
                ...
                <div class="mb-4 sm:mb-6">
                    <span
                        ...
                        Question {{ index + 1 }} of {{ profilingStore.varkQuestions.length }}
                    </span>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 md:gap-4">
                    <label v-for="answer in question.answers" :key="answer.id"
                        class="relative flex items-center gap-2 sm:gap-3 p-2 sm:p-2.5 md:p-3 border rounded-lg cursor-pointer transition-all duration-200 hover:border-blue-400 hover:bg-blue-50 min-h-[60px] sm:min-h-[70px] md:min-h-[80px]"
                        :class="profilingStore.varkResponses[question.id] === answer.id
                        ...
                        <input type="radio" :name="`q-${question.id}`" :value="answer.id"
                            ...
                            :checked="profilingStore.varkResponses[question.id] === answer.id"
                            @change="selectAnswer(question.id, answer.id)" />

                        <div class="flex-1 pr-6 sm:pr-8">
                            <div class="text-gray-800 leading-relaxed text-xs sm:text-sm md:text-base">
                                <div>{{ answer.answer_text }}</div>
                            </div>
                        </div>

                        <div v-if="profilingStore.varkResponses[question.id] === answer.id"
